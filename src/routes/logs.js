...
router.post('/:climberId', async (req, res) => {
  const { id, date, type, support, minutes, intensity, shape, location, notes, ascents, bNoGrade, planned, bankRef, cycleId, cycleName, source, assignedByCoachId, flexGoal, objectiveId, injury, injuryNote, customName, checklistDone } = req.body;
  if (!id || !date) return res.status(400).json({ error: 'id et date requis' });
  try {
    const { rows } = await pool.query(
      `INSERT INTO logs (id, climber_id, date, type, support, minutes, intensity, shape, location, notes, ascents, b_no_grade, planned, bank_ref, cycle_id, cycle_name, source, assigned_by_coach_id, flex_goal, objective_id, injury, injury_note, custom_name, checklist_done)
       ...
       ON CONFLICT (id) DO UPDATE SET
         ...
         source=$17, assigned_by_coach_id=$18, flex_goal=$19, objective_id=$20, injury=$21, injury_note=$22, custom_name=$23, checklist_done=$24, updated_at=NOW()
       RETURNING *`,
      [id, req.params.climberId, date, type, support||'', minutes||90, intensity||3,
       ...
       !!injury, injuryNote||'', customName||'', JSON.stringify(checklistDone||[])]
    );
      ...
      await client.query(
        `INSERT INTO logs (id, climber_id, date, type, support, minutes, intensity, shape, location, notes, ascents, b_no_grade, planned, bank_ref, cycle_id, cycle_name, source, assigned_by_coach_id, flex_goal, objective_id, injury, injury_note, custom_name, checklist_done)
         ...
         ON CONFLICT (id) DO UPDATE SET
           ...
           objective_id=$20, injury=$21, injury_note=$22, custom_name=$23, checklist_done=$24, updated_at=NOW()`,
        [log.id, req.params.climberId, log.date, log.type, log.support||'',
         ...
         !!log.injury, log.injuryNote||'', log.customName||'', JSON.stringify(log.checklistDone||[])]
      );
    }
